...
class Block:
    ...
    def description(self):
        if self.win.PL:
            return u"Są dwa klocki\nPierwszy z nich ma masę 1kg\nMożesz wybrać masę i prędkość drugiego z nich\n" \
               ...
               u"uderzył o ścianę\nNajciekawsze wyniki otrzymujemy dla masy drugiego klocka\n" \
               u" będącej wielokrotnością 100 kg"
        else:
    ...
    def prepare(self):
        ...
        self.block1 = box(pos=(5, 1*scale, 0), size=(2*scale, 2*scale, 0.06), color=color.blue)
        ...
        if self.win.PL:
            self.win.var_exec[0].SetLabel('uderzenia: 0')
        else:
    ...
    def run(self):
        ...
        while True:
            if self.win.simulation_stopped is False:
                rate(1000)
                if abs(self.block1.pos.x - self.block2.pos.x - 1-(self.block1.size.x/2)) < 10**-1:
                    self.counter += 1
                    tmp = self.block1.v
                    ...
                    if self.win.PL:
                        self.win.var_exec[0].SetLabel('uderzenia: ' + '%d' % self.counter)
                    else:
                        self.win.var_exec[0].SetLabel('hits: ' + '%d' % self.counter)
                if abs(self.block2.pos.x + 19) < 10**-1:
                    self.counter += 1
                    if self.win.PL:
                        self.win.var_exec[0].SetLabel('uderzenia: ' + '%d' % self.counter)
                    else:
